...
(define (delete-least-in-D D)
  (merge (non-empty-left D) (non-empty-right D)))

;; merge : D D -> D
;; Combines two Ds into a single D containing all their elements.
;; The size of D1 must be at most one more than the size of D2.
(define (merge D1 D2)
  (cond
    ...
    [else
     (cond
       ...
       [else
        ...
        (define D3
          (cond
            [(< (size-of-D D1) (size-of-D D21)) (merge D1 D21)]
            [else (merge D21 D1)]))
        (cond
          [(< (size-of-D D3) (size-of-D D22))
           (node k2 D3 D22)]
          [else
           (node k2 D22 D3)])])]))

;; Added after lecture, just to finish up:
(define (empty-D? D) (empty? D))
